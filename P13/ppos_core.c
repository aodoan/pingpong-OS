...
int task_init(task_t *task, void (*start_func)(void *), void *arg)
{
    ...
    if (stack)
    {
        ...
        if (task == Dispat || start_func == diskDriverBody){
...
int task_wait(task_t *task)
{
    ...
    #ifdef DEBUG
    printf("task_wait: tarefa %i esperando por tarefa %i\n", CurrentTask->id, task->id);
    #endif
...
{
#ifdef DEBUG
    printf("task_sleep: tarefa %i dormindo por %i ms\n", CurrentTask->id, t);
#endif
...
void awake_tasks_time()
{
    ...
    if (queueS)
    {
        do
        {
            ...
            if ((aux->wake_time <= TIME && aux->wait_id < 0) && (aux->f != 1))
            {
                aux2 = aux;
                if (aux == aux->next)
                {
...
{
#ifdef DEBUG
    printf("sem_init: iniciando o semaforo com valor de %i\n", value);
#endif
    s->value = value;
...
int mqueue_init(mqueue_t *queue, int max_msgs, int msg_size)
{
    ...
    /* start the buffer as a vector of max_msgs * msg_size bytes */
...
/*
    auxiliar functions (used in ppos_disk)
*/
void wakeTask(task_t * task){
    task_resume(task, &queueS);
}
